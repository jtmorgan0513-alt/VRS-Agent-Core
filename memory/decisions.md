...
Record significant technical decisions here. Use the format below.
...
**Consequences**: All display code (agent dashboard, tech detail, CSV export) must handle both formats.
...
**Decision**: Fire-and-forget `POST /api/uploads/report-error` endpoint that just `console.error("[UPLOAD-DIAGNOSTIC]", ...)`. No database table, no retention policy.
...
**Date**: 2026-04-16
**Status**: Active
**Context**: Admins had no online/offline toggle, so their `agentStatus` defaulted to "offline" and they were excluded from all broadcasts.
**Decision**: Broadcast functions check `isAdmin` separately — admins skip status check entirely and always receive notifications when connected via WebSocket.
